    ...
    >
      <AnimatePresence mode="sync">
        <motion.div key={slide.id} initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.8 }} className="absolute inset-0">
          {/* SEO FIX: meaningful alt text, not empty string */}
          <img
            src={slide.image_url}
      ...
      <div className="relative z-10 h-full container mx-auto px-4 flex items-center">
        <AnimatePresence mode="wait">
          <motion.div key={slide.id} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }} className="max-w-2xl">
            ...
            {/* SEO FIX: h1 is already in HeroSection — correct placement */}
            <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-heading font-bold leading-tight mb-6" style={{ color: "white" }}>
              {slide.title}
            </h1>
            <p className="text-lg sm:text-xl mb-8 max-w-xl" style={{ color: "rgba(255,255,255,0.8)" }}>
              {slide.subtitle}
            </p>
            <div className="flex flex-wrap gap-4">
              <Button
                ...
                asChild
              >
                <a href={slide.cta_link} aria-label={`${slide.cta_text} — ATEC Gurdaspur`}>
                  {slide.cta_text}
                </a>
              </Button>
              <Button
                ...
                asChild
              >
                <a href="#videos" aria-label="Watch ATEC demo video">
                  <Play className="w-4 h-4 mr-2" aria-hidden="true" />Watch Demo
                </a>
              </Button>
            </div>
          </motion.div>
      ...
      <div className="absolute bottom-8 left-0 right-0 z-10 container mx-auto px-4 flex items-center justify-between">
        ...
        <div className="flex gap-2" role="group" aria-label="Slide navigation">
          ...
          >
            <ChevronLeft className="w-5 h-5" style={{ color: "white" }} aria-hidden="true" />
          </button>
          <button
            ...
            className="w-10 h-10 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center hover:bg-white/20 transition-colors"
          >
            <ChevronRight className="w-5 h-5" style={{ color: "white" }} aria-hidden="true" />
          </button>
        </div>
      </div>
    </section>
